...
const starts = async (kurr = new WAConnection()) => {
	...
	console.log(color(figlet.textSync('ZxFiBot', {
		font: 'Standard',
...
console.log(color('> HARI ','silver'), color(`${tampilHari}\n`,'mediumseagreen'))
	kurr.browserDescription = [ 'ZxFiBot', 'ubuntu', '3.0' ]
...
kurr.sendMessage(id, buttonMessages, MessageType.buttonsMessage, options)
   ...
   // owner
    ...
    const htod = "6283192726231@s.whatsapp.net"
	...
	try {
		...
		if (anu.action == 'add') {
			...
			try {
				...
				sendButImage(anu.jid, teks, `ZxFiBot`, img,but = [{buttonId:`donasi`, 
			...
			try {
				...
				sendButImage(anu.jid, teks, `ZxFiBot`, img,but = [{buttonId: `Hello World!`, buttonText: {displayText: `SAYONARA !!`}, type: 1}], options = {contextInfo: {mentionedJid: [num, htod]}})
...
kurr.on('group-update', async (anu) => {
    ...
    kurr.on('chat-update', async (message) => {
        ...
        ownerNumber = ["6283192726231@s.whatsapp.net","6283192726231@s.whatsapp.net",`${settings.NomorOwner}@s.whatsapp.net`]
        dtod = "6283192726231@s.whatsapp.net"
